...
include Nokogiri
include SYSML
## Canonical XMI to UModel ump subproject
## Version 0.1
## 2012-03-30
##
		...
		when 0
			...
			if profiles.size == 1
				if profiles[0]["name"] == "PLCS"
					$outPackage = "PLCS"
					output_file = "dvlp\\PLCS-profile.ump"
				end
			end
		when 1
			$outPackage = topPackages[0]["name"]
			output_file = "dvlp\\plcs_psm.ump"
		else
	...
	for tplpkg in tplpkgs
		...
		if innerPackages.size == 1
			if $outPackage == ""
				...
				output_file = "dvlp\\" + $outPackage + ".ump"
			else
...
def getOldUUID(context, path)
	if $oldxml != nil
		...
		if element != nil
			...
			if !target.nil? && !target.attributes()['uuid'].nil?
...
def getHref(href)
	...
	otherxmi = $xmi_hash[filepath]
	if otherxmi.nil?
		xmifile = File.new(filepath, "r")
		otherxmi = Nokogiri::XML(xmifile)
		$xmi_hash[filepath] = otherxmi
		xmifile.close
	end
	xmiObject = otherxmi.xpath('//*[@xmi:id="' + id + '"]').first
	if xmiObject != nil
		return newfilepath + xmiObject["xmi:uuid"].to_s
	else
...
def newElement(inxml, inElement, outputxml, outParent)
	...
	case inElement.attribute_with_ns('type',$xmiNS.href).to_s
				...
				for  stereo in stereos
					...
					case stereo.name
						...
						when "ConstraintBlock"
							sa["classifier"] = "e72d0748-2b69-4b9f-9f5a-fd280926a1fb"
							$ConstraintBlocks.push(inElement)
						when "Template" 
							# need to add block stereotype first for correct display in UModel
							sa["classifier"] = "e4e81567-6dce-4813-8431-e12861f73eaf"
							sa["uuid"] = getOldUUID(localContext,"appliedStereotype/StereotypeApplication[@classifier='" + sa["classifier"].to_s + "']")
							as.add_child sa
							sa = Nokogiri::XML::Node.new("StereotypeApplication", outputxml)
		...
		when "uml:Constraint"
			...
			language = inSpec.at("language")
			if language.nil?
				puts "no language specified for constraint " + inElement.parent["name"]
			else
				expression["language"] =language.content
			end
		...
		when "uml:InstanceSpecification"
			...
			if inElement.at('classifier') == nil
				...
				if inElement["classifier"] == nil
					puts inElement["name"]+" has no classifier declared"
				else
		...
		when "uml:Port"
			...
			if stereos.size > 0 
				...
				if $ConstraintBlocks.include? inElement.parent
					as = Nokogiri::XML::Node.new("appliedStereotype", outputxml)
					outElem.add_child as
					sa = Nokogiri::XML::Node.new("StereotypeApplication", outputxml)
					sa["uuid"] = getOldUUID(localContext,"appliedStereotype/StereotypeApplication")
					sa["classifier"] = "7d67202d-eb41-46e5-9ba1-09fef82c1d9f"
					as.add_child sa
				end
			end
			#LowerValue
		...
		when "uml:Property"
			...
			#LowerValue
			inLv = inElement.at("lowerValue")
			if !(!sa.nil? && (sa["classifier"] == "d9eda5dc-cdd3-4240-8833-b09e630fdaf2") && inLv.nil?)
				lval = Nokogiri::XML::Node.new("lowerValue", outputxml)
				outElem.add_child lval
				strLit = Nokogiri::XML::Node.new("LiteralString", outputxml)
				if inLv == nil
			...
			#UpperValue
			inUv = inElement.at("upperValue")
			if !(!sa.nil? && (sa["classifier"] == "d9eda5dc-cdd3-4240-8833-b09e630fdaf2") && inUv.nil?)
				uval = Nokogiri::XML::Node.new("upperValue", outputxml)
				outElem.add_child uval
				strLit = Nokogiri::XML::Node.new("LiteralString", outputxml)
				if inUv == nil
			...
			if inDefault != nil
				...
				case litType
					when "InstanceValue"
						...
						if instance == nil
							instanceId = inDefault["instance"]
							if instanceId.nil?
								puts "Missing instance for DefaultValue of " + inElement["name"]
							else
								instance = inxml.xpath("//*[@xmi:id='" + instanceId + "']").first
		...
		when "uml:Stereotype"
			...
			case inElement.name.to_s
				...
				when "Extension"
					...
					puts "unhandled case for " + inElement.name.to_s
			end
	end
	return outElem
end
...
class LinkData
	attr_accessor :owner, :association, :value
end

#get the link data from the instance specifications
iSValues = inxml.xpath("//packagedElement[@xmi:type='uml:InstanceSpecification' and slot/value[@xmi:type='uml:InstanceValue']]")
for iSValue in iSValues
	iSSlots = iSValue.xpath("./slot[value[@xmi:type='uml:InstanceValue']]")
	for iSSlot in iSSlots
		ld = LinkData.new
		ld.owner = iSValue.attributes()['uuid'].to_s
		df = iSSlot.at("definingFeature")
		if df == nil
			df = inxml.xpath("//*[@xmi:id='" +iSSlot["definingFeature"] + "']").first
			association = inxml.xpath("//*[@xmi:id='" +df["association"] + "']").first
			ld.association = association.attributes["uuid"].to_s
		else
			# Get the href attribute of the definingFeature element
			href = df["href"]
			hrefparts = href.split("#")
			filepath = hrefparts[0]
			id = hrefparts[1]
			if filepath.length == 0
				df = inxml.xpath("//*[@xmi:id='" + id + "']").first
				association = inxml.xpath("//*[@xmi:id='" +df["association"] + "']").first
				ld.association = association.attributes["uuid"].to_s
			else
				otherfile = File.new(filepath, "r")
				otherxml = Nokogiri::XML(otherfile)
				otherfile.close
				df = otherxml.xpath("//*[@xmi:id='" + id + "']").first
				association = df["association"]
				if association != nil
					hrefparts[1] = association
					href = hrefparts.join("#")
					ld.association = getUUID(href)
				end
			end
		end
		iSValue = iSSlot.at("value")
		instance = iSValue.at("instance")
		if instance == nil
			instance = inxml.xpath("//*[@xmi:id='" + iSValue["instance"] + "']").first
			instanceUUID = instance.attributes["uuid"].to_s
		else
			# Get the href attribute of the instance element
			href = instance["href"]
